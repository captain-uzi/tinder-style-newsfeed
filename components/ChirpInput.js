...
export const PATTERN_MENTION = "(^|s)@[a-z_d-]+";
export const PATTERN_HASHTAG = "(^|s)#[a-z_d-]+";
...
const SegmentedTextInput = React.forwardRef(
  (
    ...
    const [debouncedLoading] = useDebounce(
      loadingSuggestions,
      suggestionDebounce * 0.5
    );
    const [debouncedSuggestions, { flush }] = useDebounce(
      suggestions,
      suggestionDebounce * 0.5
    );
      ...
      return ((typeCheck("String", value) && value) || "")
        ...
        .map((str) => [
          ...
          Object.keys(patterns).reduce(
            (selected, regExp) =>
              selected || (new RegExp(regExp, "gm").test(str) && regExp),
            null
          ),
        ]);
    }, [value, patterns]);
    ...
    const onChangeTextCallback = useCallback(
      (nextValue) => {
        ...
        if (!isEqual(value, nextValue)) {
          const nextSegments = [...segments, ...validSegments].filter(
    ...
    const renderLastSegmentAsInvalid =
      lastSegmentText.length > 0 &&
      !isValidLastSegment &&
      !!shouldRenderInvalid(lastSegmentText);
    ...
    useEffect(() => {
      if (
        !shouldDisable &&
        !renderLastSegmentAsInvalid &&
        lastSegmentText.length >= minSuggestionLength
      ) {
        debouncedSuggestion.cancel();
    ...
    const renderedSegments = React.useMemo(() => {
      return segmentsToRender.map(([str, regexp], i) => {
        const Component =
          regexp === false
            ? patterns[""] || React.Fragment
            : patterns[regexp] || React.Fragment;
        return (
          <Component
            key={str}
            style={textStyle}
    ...
    React.useEffect(() => {
      if (
        (renderLastSegmentAsInvalid || !value.length) &&
    ...
    const shouldPickSuggestion = React.useCallback(
      ([suggestion, regexp]) => {
...
SegmentedTextInput.propTypes = {
  ...
  segmentContainerStyle: PropTypes.oneOfType([
    PropTypes.shape({}),
    PropTypes.number,
  ]),
  suggestionsContainerStyle: PropTypes.oneOfType([
    PropTypes.shape({}),
    PropTypes.number,
  ]),
...
SegmentedTextInput.defaultProps = {
  ...
  patterns: {
    /* a twitter @mention */
    [""]: ({ style, onRequestDelete, children, ...extraProps }) => (
    ...
    [PATTERN_MENTION]: ({
      ...
      <TouchableOpacity onPress={onRequestDelete}>
        <Text
          {...extraProps}
          style={[style, { fontWeight: "bold", color: "blue" }]}
          children={`${children} `}
        />
      </TouchableOpacity>
    ),
    [PATTERN_HASHTAG]: ({
      ...
      <TouchableOpacity onPress={onRequestDelete}>
        <Text
          {...extraProps}
          style={[style, { fontWeight: "bold", color: "green" }]}
          children={`${children} `}
        />
      </TouchableOpacity>
    ),
  ...
  shouldRenderInvalid: (str) => false, //!str.startsWith("@"),
  max: 50,
    ...
    >
      ...
      {suggestions.map((suggestion, i) => (
        <TouchableOpacity
          ...
          onPress={() =>
            pickSuggestion([suggestion, PATTERN_MENTION.toString()])
          }
        >
          <Text
            style={{
